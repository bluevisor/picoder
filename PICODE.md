...
build.sh              cross-compile (and `build.sh deploy` to install on the Pi)
src/
...
- Composer: `@file` attach, Tab autocomplete (commands + paths), history,
  word-skip (Alt/Ctrl/Cmd + arrows), code-block highlighting.
...
./build.sh deploy    # build + scp to the Pi at ~/.local/bin/picode
```
